"""Entry point train task mouth. Chạy:
    python training/train_mouth.py
    python training/train_mouth.py --epochs 20 --batch-size 64
"""
...
def main() -> None:
    ...
    train_loader, val_loader, train_ds, val_ds = get_mouth_dataloaders(cfg)
    print(f"Train: {len(train_ds)} ảnh | Val: {len(val_ds)} ảnh | classes: {train_ds.class_to_idx}")

    model = build_mouth_model(device)
