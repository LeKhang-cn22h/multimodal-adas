"""Frame consumer for Driver-Service.

Consumes frames from the driver.frames queue, decodes JPEG,
runs fatigue detection, and publishes results.
"""
...
class FrameConsumer:
    """Consumes JPEG frames from RabbitMQ and processes them.

    Runs in a background thread. For each consumed frame:
    1. Extracts JPEG bytes from message body
    2. Reads frame_id and timestamp from headers
    3. Runs FatigueDetector.process()
    4. Publishes result via ResultPublisher

    Never blocks the FastAPI request/response cycle.
    """

    EXCHANGE: str = "adas.exchange"
    EXCHANGE_TYPE: str = "topic"
    QUEUE: str = "driver.frames"
    ROUTING_KEY: str = "driver.frame"
    ...
    def _on_frame_message(
        ...
        """Process an incoming frame message."""
