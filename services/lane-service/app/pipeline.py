...
class LanePipeline:
    ...
    def process_frame(self, frame, visualize: bool = False) -> dict:
        ...
        # 4. Phân tích hình học đường làn (Geometry) - Sliding Window + Perspective Warp
        lane_info = self.geometry.analyze_lane(lane_mask, orig_frame=frame if visualize else None)
        ...
        if visualize:
            # Ưu tiên dùng overlay từ LaneGeometry (sliding window + inverse warp)
            overlay_frame = lane_info.get("overlay_frame")
            if overlay_frame is not None:
                # Copy overlay vào frame gốc
                frame[:] = overlay_frame[:]
            else:
                # Fallback: tô màu drivable area + vẽ đường thẳng đơn giản
                overlay = frame.copy()
                ...
                if left_line:
                    cv2.line(frame, left_line[0], left_line[1], (255, 80, 80), 3, cv2.LINE_AA)
                if right_line:
                    cv2.line(frame, right_line[0], right_line[1], (80, 80, 255), 3, cv2.LINE_AA)

            # Luôn vẽ YOLO boxes (đè lên overlay)
