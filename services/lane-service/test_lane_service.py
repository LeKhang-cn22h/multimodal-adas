...
def run_tests():
    ...
    # ---------------------------------------------------------------------------
    # TEST 1: Health check (với tối đa 3 lần thử lại khi khởi động)
    # ---------------------------------------------------------------------------
    print("\n[1] GET /health")
    import time
    for attempt in range(5):
        try:
            ...
            if attempt == 4:
                check("Health reachable", False, f"Failed after 5 attempts: {str(e)}")
            else:
                print(f"  [*] Service is starting... retrying in 2 seconds (attempt {attempt+1}/5)")
                time.sleep(2)
